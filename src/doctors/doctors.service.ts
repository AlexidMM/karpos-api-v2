...
export class DoctorsService {
    ...
    async deleteDoctor(id: number) {
        await this.database.delete(schema.doctors).where(eq(schema.doctors.id_dc, id));
    }

    async updateDoctor(id: number, updates: Partial<typeof schema.doctors.$inferInsert>) {
        await this.database.update(schema.doctors)
            .set(updates)
