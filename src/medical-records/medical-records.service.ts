...
export class MedicalRecordsService {
    ...
    async createMedicalRecord(record: typeof schema.medical_records.$inferInsert) {
        await this.database.insert(schema.medical_records).values(record);
    }

    async deleteMedicalRecord(id: number) {
        await this.database.delete(schema.medical_records).where(eq(schema.medical_records.id_mr, id));
    }

    async updateMedicalRecord(id: number, updates: Partial<typeof schema.medical_records.$inferInsert>) {
        await this.database.update(schema.medical_records)
            .set(updates)
